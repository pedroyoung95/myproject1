...
<div>	
	<c:forEach items="${subReplyList }" var="subReply" varStatus="status">
		...
		<div>
			...
			<input type="text" name="contentNo" value="${contentData.content.number }" hidden/>
			<input type="text" name="replyId" value="${reply.id }" hidden/>  
			...
			<span>${subReply.registerid }</span>
			 <c:if test="${authUser.id == subReply.registerid}">
		        <div class="button-container mt-3">
        		  ...
        		  <a class="btn btn-danger" href="${pageContext.request.contextPath }/subReply/remove.do?no=${subReply.num }&contentNo=${contentData.content.number}"><i class="fas fa-trash-alt"></i> 삭제</a>
		        </div>		        
		      </c:if>
		      </form>	
		</div>
