	...
	<c:forEach items="${subReplyList }" var="subReply" varStatus="status">
		<div>
			...
			<c:if test="${errors.noPermission_sub }">
				<small class="form-text text-muted">수정 권한이 없습니다.</small>			
			</c:if>
			<input type="text" name="contentNo" value="${param.no }" hidden/>  
			...
			<span>${subReply.registerid }</span>
			 <c:if test="${authUser.id == subReply.registerid}">
		        <div class="button-container mt-3">
        		  ...
        		  <a class="btn btn-danger" href="${pageContext.request.contextPath }/subReply/remove.do?no=${subReply.num }&contentNo=${param.no}"><i class="fas fa-trash-alt"></i> 삭제</a>
		        </div>		        
		      </c:if>
		      </form>	
		</div>
